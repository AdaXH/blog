...
export default {
  ...
  reducers: {
    ...
    dynamicDetail(state, { payload = {} }) {
      return { ...state, dynamicDetail: { visible: true, ...payload } };
    },
    closeDetail(state) {
      return {
        ...state,
  ...
  effects: {
    *addDynamic({ title, content, img, date, upvote }, { put, call }) {
      const src =
        img === 'admin'
          ? 'http://adaxh.applinzi.com/resouce/gallery/7.jpg'
          : img;
      const result = yield call(Api, 'api/addDynamic', 'POST', {
        title,
        content,
        img: src,
        date,
    ...
    *updateDynamic({ title, content, img, _id }, { put, call }) {
      const result = yield call(Api, 'api/updateDynamic', 'POST', {
        title,
        content,
        img,
        _id,
    ...
    *leaveMsg(action, { call, put }) {
      const {
        payload: { msg, _id, name },
      } = action;
      const result = yield call(Api, 'api/discussDynamic', 'POST', {
        msg,
        _id,
        name,
      });
