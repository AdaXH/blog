...
routerExports.login = {
  ...
  route: async (ctx, next) => {
    ...
    try {
      ...
      ctx.cookies.set('token', getToken({ _id: result._id }), {
        expires: date,
        httpOnly: false,
        overwrite: false,
      })
      ...
      function(error, response) {
        if (error) {
          ...
          User.updateOne(
            ...
            {
              $set: {
                avatar: `http://sinacloud.net/ada.bucket/avatar/${fileName}${_id}.${imgType}${accessKey}`,
              },
            }
          ).then((data) =>
            data.n === 1 ? resolve(bf) : reject('头像更新失败')
          )
        }
      }
    )
